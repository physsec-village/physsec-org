...
class TurnstileVerificationTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_valid_contact_token_is_accepted(self):
        client = self._client_with_response({"success": True, "action": "contact"})
        with (
            patch.dict(
                "os.environ",
        ...
        self.assertEqual(post.await_args.args[0], "https://challenges.cloudflare.com/turnstile/v0/siteverify")
        self.assertEqual(
    ...
    async def test_wrong_action_is_rejected(self):
        client = self._client_with_response({"success": True, "action": "other"})
        with (
            patch.dict(
                "os.environ",
                {"TURNSTILE_SITE_KEY": "site", "TURNSTILE_SECRET_KEY": "secret"},
