...
- a Let's Encrypt certificate for both `physsec.org` and `www.physsec.org`
  exists at `/etc/letsencrypt/live/physsec.org/`; and
...
sudo install -D -m 0644 deploy/nginx/physsec.org.conf \
  /etc/nginx/sites-available/physsec.org
sudo ln -sfn /etc/nginx/sites-available/physsec.org \
  /etc/nginx/sites-enabled/physsec.org
sudo nginx -t
